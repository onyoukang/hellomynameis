...
<html lang="en">
    ...
    <body>
        <table>
            ...
            <?php
            $conn = mysqli_connect("localhost", "wustl_inst", "wustl_pass", "capstone");
              // Check connection
              if ($conn->connect_error) {
               die("Connection failed: " . $conn->connect_error);
              } 
              $sql = "SELECT id, fullname, prefname FROM stories";
        ...
        <p class="helvetica">My given name is  _______________________________, but I prefer to go by _________________________.
        <?php
        $conn = mysqli_connect("localhost", "wustl_inst", "wustl_pass", "capstone");
          // Check connection
          if ($conn->connect_error) {
          ...
          if ($result->num_rows > 0) {
           // output data of each row
           while($row = $result->fetch_assoc()) {
            echo "<p>My given name is " . $row["fullname"]. ", but I prefer to go by ". $row["prefname"]. ".</p>";
        }
        echo "</table>";
        } else { echo "0 results"; }
        $conn->close();
